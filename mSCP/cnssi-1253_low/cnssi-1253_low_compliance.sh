...
audit_plist="/Library/Preferences/org.cnssi-1253_low.audit.plist"
audit_log="/Library/Logs/cnssi-1253_low_baseline.log"
...
show_menus() {
    lastComplianceScan=$(defaults read /Library/Preferences/org.cnssi-1253_low.audit.plist lastComplianceCheck)
...
reset_plist(){
    echo "Clearing results from /Library/Preferences/org.cnssi-1253_low.audit.plist"
    defaults delete /Library/Preferences/org.cnssi-1253_low.audit.plist
}
...
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey("$rule"))["exempt"]
EOS
)
            if [[ $is_exempt == "1" ]]; then
...
if [[ $(/usr/bin/tail -n 1 "$audit_log" 2>/dev/null) = *"Remediation complete" ]]; then
 	echo "$(date -u) Beginning cnssi-1253_low baseline scan" >> "$audit_log"
else
 	echo "$(date -u) Beginning cnssi-1253_low baseline scan" > "$audit_log"
fi
...
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_acls_files_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_acls_files_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_files_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_files_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_acls_files_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_files_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_acls_folders_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_acls_folders_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_folders_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_folders_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_acls_folders_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_acls_folders_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ $LAUNCHD_RUNNING == 1 ]] && [[ -e /etc/security/audit_control ]] && [[ $AUDITD_RUNNING == 1 ]]; then
  ...
  echo "fail"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_auditd_enabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_auditd_enabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "pass" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_auditd_enabled passed (Result: $result_value, Expected: "{'string': 'pass'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_auditd_enabled failed (Result: $result_value, Expected: "{'string': 'pass'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_auditd_enabled -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_auditd_enabled failed (Result: $result_value, Expected: "{'string': 'pass'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_configure_capacity_notify'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_configure_capacity_notify'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "25" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_configure_capacity_notify passed (Result: $result_value, Expected: "{'integer': 25}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_configure_capacity_notify failed (Result: $result_value, Expected: "{'integer': 25}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_configure_capacity_notify -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_configure_capacity_notify failed (Result: $result_value, Expected: "{'integer': 25}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_acls_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_acls_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_acls_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_acls_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_control_acls_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_acls_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_control_group_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_mode_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_control_mode_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_control_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_control_owner_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_control_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_failure_halt'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_failure_halt'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_failure_halt passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_failure_halt failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_failure_halt -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_failure_halt failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_files_group_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_mode_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_files_mode_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_files_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_files_owner_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_files_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_aa_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_aa_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_aa_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_aa_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_aa_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_aa_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_ad_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_ad_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ad_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ad_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_ad_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ad_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_ex_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_ex_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ex_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ex_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_ex_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_ex_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fd_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fd_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fd_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fd_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_fd_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fd_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fm_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fm_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_fm_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fm_failed_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fm_failed_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_failed_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_failed_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_fm_failed_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fm_failed_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fr_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fr_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fr_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fr_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_fr_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fr_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fw_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_fw_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fw_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fw_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_fw_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_fw_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_lo_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_flags_lo_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_lo_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_lo_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_flags_lo_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_flags_lo_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folder_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folder_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_folder_group_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folder_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folder_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_folder_owner_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folder_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folders_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_folders_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "700" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_folders_mode_configure passed (Result: $result_value, Expected: "{'integer': 700}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folders_mode_configure failed (Result: $result_value, Expected: "{'integer': 700}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_folders_mode_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_folders_mode_configure failed (Result: $result_value, Expected: "{'integer': 700}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_retention_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_retention_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "7d" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_retention_configure passed (Result: $result_value, Expected: "{'string': '7d'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '7d'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_retention_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '7d'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_settings_failure_notify'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('audit_settings_failure_notify'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - audit_settings_failure_notify passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_settings_failure_notify failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" audit_settings_failure_notify -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - audit_settings_failure_notify failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_login_smartcard_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_login_smartcard_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_login_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_login_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_pam_login_smartcard_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_login_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_su_smartcard_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_su_smartcard_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_su_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_su_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_pam_su_smartcard_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_su_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_sudo_smartcard_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_pam_sudo_smartcard_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_sudo_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_sudo_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_pam_sudo_smartcard_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_pam_sudo_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_allow'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_allow'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_allow passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_allow failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_smartcard_allow -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_allow failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_certificate_trust_enforce_moderate'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_certificate_trust_enforce_moderate'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_certificate_trust_enforce_moderate passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_certificate_trust_enforce_moderate failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_smartcard_certificate_trust_enforce_moderate -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_certificate_trust_enforce_moderate failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_smartcard_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_smartcard_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_smartcard_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_ssh_password_authentication_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('auth_ssh_password_authentication_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - auth_ssh_password_authentication_disable passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_ssh_password_authentication_disable failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" auth_ssh_password_authentication_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - auth_ssh_password_authentication_disable failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_addressbook_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_addressbook_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_addressbook_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_addressbook_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_addressbook_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_addressbook_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_appleid_system_settings_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_appleid_system_settings_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_appleid_system_settings_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_appleid_system_settings_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_appleid_system_settings_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_appleid_system_settings_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_bookmarks_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_bookmarks_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_bookmarks_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_bookmarks_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_bookmarks_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_bookmarks_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_calendar_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_calendar_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_calendar_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_calendar_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_calendar_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_calendar_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_drive_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_drive_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_drive_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_drive_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_drive_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_drive_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_freeform_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_freeform_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_freeform_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_freeform_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_freeform_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_freeform_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_game_center_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_game_center_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_game_center_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_game_center_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_game_center_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_game_center_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_keychain_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_keychain_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_keychain_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_keychain_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_keychain_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_keychain_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_mail_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_mail_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_mail_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_mail_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_mail_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_mail_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_notes_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_notes_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_notes_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_notes_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_notes_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_notes_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_photos_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_photos_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_photos_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_photos_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_photos_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_photos_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_private_relay_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_private_relay_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_private_relay_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_private_relay_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_private_relay_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_private_relay_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_reminders_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_reminders_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_reminders_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_reminders_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_reminders_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_reminders_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_sync_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('icloud_sync_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - icloud_sync_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_sync_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" icloud_sync_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - icloud_sync_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_account_modification_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_account_modification_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_account_modification_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_account_modification_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_account_modification_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_account_modification_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_airdrop_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_airdrop_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_airdrop_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_airdrop_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_airdrop_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_airdrop_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_appleid_prompt_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_appleid_prompt_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_appleid_prompt_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_appleid_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_appleid_prompt_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_appleid_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_asl_log_files_owner_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_asl_log_files_owner_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_owner_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_owner_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_asl_log_files_owner_group_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_owner_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_asl_log_files_permissions_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_asl_log_files_permissions_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_permissions_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_permissions_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_asl_log_files_permissions_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_asl_log_files_permissions_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_authenticated_root_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_authenticated_root_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_authenticated_root_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_authenticated_root_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_bluray_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_bluray_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_bluray_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_bluray_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_blank_bluray_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_bluray_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_cd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_cd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_cd_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_cd_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_blank_cd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_cd_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_dvd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_blank_dvd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_dvd_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_dvd_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_blank_dvd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_blank_dvd_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_bluray_read_only_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_bluray_read_only_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "read-only" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_bluray_read_only_enforce passed (Result: $result_value, Expected: "{'string': 'read-only'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_bluray_read_only_enforce failed (Result: $result_value, Expected: "{'string': 'read-only'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_bluray_read_only_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_bluray_read_only_enforce failed (Result: $result_value, Expected: "{'string': 'read-only'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_bonjour_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_bonjour_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_bonjour_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_bonjour_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_bonjour_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_bonjour_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_burn_support_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_burn_support_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_burn_support_disable passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_burn_support_disable failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_burn_support_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_burn_support_disable failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_calendar_app_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_calendar_app_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_calendar_app_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_calendar_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_calendar_app_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_calendar_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_cd_read_only_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_cd_read_only_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "read-only" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_cd_read_only_enforce passed (Result: $result_value, Expected: "{'string': 'read-only'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_cd_read_only_enforce failed (Result: $result_value, Expected: "{'string': 'read-only'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_cd_read_only_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_cd_read_only_enforce failed (Result: $result_value, Expected: "{'string': 'read-only'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_config_data_install_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_config_data_install_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_config_data_install_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_config_data_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_config_data_install_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_config_data_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_config_profile_ui_install_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_config_profile_ui_install_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_config_profile_ui_install_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_config_profile_ui_install_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_config_profile_ui_install_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_config_profile_ui_install_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_disk_image_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_disk_image_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_disk_image_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_disk_image_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_disk_image_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_disk_image_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_dvdram_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_dvdram_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_dvdram_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_dvdram_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_dvdram_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_dvdram_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_erase_content_and_settings_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_erase_content_and_settings_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_erase_content_and_settings_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_erase_content_and_settings_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_erase_content_and_settings_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_erase_content_and_settings_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_facetime_app_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_facetime_app_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_facetime_app_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_facetime_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_facetime_app_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_facetime_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_filevault_autologin_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_filevault_autologin_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_filevault_autologin_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_filevault_autologin_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_filevault_autologin_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_filevault_autologin_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firewall_default_deny_require'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firewall_default_deny_require'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_default_deny_require passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_default_deny_require failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_firewall_default_deny_require -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_default_deny_require failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firewall_log_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firewall_log_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_log_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_log_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_firewall_log_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firewall_log_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firmware_password_require'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_firmware_password_require'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_firmware_password_require passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firmware_password_require failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_firmware_password_require -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_firmware_password_require failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_gatekeeper_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_gatekeeper_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_gatekeeper_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_gatekeeper_rearm'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_gatekeeper_rearm'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_rearm passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_rearm failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_gatekeeper_rearm -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_gatekeeper_rearm failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_handoff_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_handoff_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_handoff_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_handoff_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_handoff_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_handoff_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_home_folders_secure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_home_folders_secure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_home_folders_secure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_home_folders_secure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_home_folders_secure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_home_folders_secure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_httpd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_httpd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_httpd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_httpd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_httpd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_httpd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_icloud_storage_prompt_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_icloud_storage_prompt_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_icloud_storage_prompt_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_icloud_storage_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_icloud_storage_prompt_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_icloud_storage_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_install_log_retention_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_install_log_retention_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "Yes" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_install_log_retention_configure passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_install_log_retention_configure failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_install_log_retention_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_install_log_retention_configure failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ir_support_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ir_support_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_ir_support_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ir_support_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_ir_support_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ir_support_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_mail_app_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_mail_app_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_mail_app_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_mail_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_mail_app_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_mail_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_mdm_require'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_mdm_require'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_mdm_require passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_mdm_require failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_mdm_require -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_mdm_require failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_messages_app_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_messages_app_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_messages_app_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_messages_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_messages_app_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_messages_app_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_newsyslog_files_owner_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_newsyslog_files_owner_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_owner_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_owner_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_newsyslog_files_owner_group_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_owner_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_newsyslog_files_permissions_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_newsyslog_files_permissions_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_permissions_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_permissions_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_newsyslog_files_permissions_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_newsyslog_files_permissions_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_nfsd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_nfsd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_nfsd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_nfsd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_nfsd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_nfsd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_on_device_dictation_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_on_device_dictation_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_on_device_dictation_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_on_device_dictation_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_on_device_dictation_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_on_device_dictation_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_parental_controls_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_parental_controls_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_parental_controls_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_parental_controls_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_parental_controls_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_parental_controls_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_autofill_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_autofill_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_password_autofill_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_autofill_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_password_autofill_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_autofill_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_hint_remove'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_hint_remove'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_password_hint_remove passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_hint_remove failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_password_hint_remove -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_hint_remove failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_proximity_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_proximity_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_password_proximity_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_proximity_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_password_proximity_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_proximity_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_password_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_password_sharing_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_sharing_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_password_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_password_sharing_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_loginwindow_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_loginwindow_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_loginwindow_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_loginwindow_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_policy_banner_loginwindow_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_loginwindow_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
test "$(cat /etc/banner)" = "$bannerText" && echo "1" || echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_ssh_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_ssh_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_policy_banner_ssh_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_ssh_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_policy_banner_ssh_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_policy_banner_ssh_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_policy_banner_ssh_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_power_nap_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_power_nap_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_power_nap_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_power_nap_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_power_nap_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_power_nap_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_privacy_setup_prompt_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_privacy_setup_prompt_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_privacy_setup_prompt_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_privacy_setup_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_privacy_setup_prompt_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_privacy_setup_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_rapid_security_response_allow'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_rapid_security_response_allow'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_allow passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_allow failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_rapid_security_response_allow -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_allow failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_rapid_security_response_removal_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_rapid_security_response_removal_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_removal_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_removal_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_rapid_security_response_removal_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_rapid_security_response_removal_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_removable_media_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_removable_media_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "deny" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_removable_media_disable passed (Result: $result_value, Expected: "{'string': 'deny'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_removable_media_disable failed (Result: $result_value, Expected: "{'string': 'deny'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_removable_media_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_removable_media_disable failed (Result: $result_value, Expected: "{'string': 'deny'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_root_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_root_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_root_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_root_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_root_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_root_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_screensaver_loginwindow_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_screensaver_loginwindow_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "Ventura" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_screensaver_loginwindow_enforce passed (Result: $result_value, Expected: "{'string': 'ventura'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: "{'string': 'ventura'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_screensaver_loginwindow_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: "{'string': 'ventura'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sip_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sip_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sip_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sip_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sip_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sip_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_siri_prompt_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_siri_prompt_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_siri_prompt_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_siri_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_siri_prompt_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_siri_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_skip_screen_time_prompt_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_skip_screen_time_prompt_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_screen_time_prompt_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_screen_time_prompt_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_skip_screen_time_prompt_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_screen_time_prompt_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_skip_unlock_with_watch_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_skip_unlock_with_watch_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_unlock_with_watch_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_unlock_with_watch_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_skip_unlock_with_watch_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_skip_unlock_with_watch_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
/usr/bin/grep -c "$fips_ssh_config" /etc/ssh/ssh_config.d/fips_ssh_config
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_fips_compliant'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_fips_compliant'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "8" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_fips_compliant passed (Result: $result_value, Expected: "{'integer': 8}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_fips_compliant failed (Result: $result_value, Expected: "{'integer': 8}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_ssh_fips_compliant -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_fips_compliant failed (Result: $result_value, Expected: "{'integer': 8}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
/bin/echo $ret
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_server_alive_count_max_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_server_alive_count_max_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "pass" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_count_max_configure passed (Result: $result_value, Expected: "{'string': 'pass'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_count_max_configure failed (Result: $result_value, Expected: "{'string': 'pass'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_ssh_server_alive_count_max_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_count_max_configure failed (Result: $result_value, Expected: "{'string': 'pass'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
/bin/echo $ret
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_server_alive_interval_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_ssh_server_alive_interval_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "pass" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_interval_configure passed (Result: $result_value, Expected: "{'string': 'pass'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_interval_configure failed (Result: $result_value, Expected: "{'string': 'pass'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_ssh_server_alive_interval_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_ssh_server_alive_interval_configure failed (Result: $result_value, Expected: "{'string': 'pass'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_channel_timeout_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_channel_timeout_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "900" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_channel_timeout_configure passed (Result: $result_value, Expected: "{'integer': 900}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_channel_timeout_configure failed (Result: $result_value, Expected: "{'integer': 900}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_channel_timeout_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_channel_timeout_configure failed (Result: $result_value, Expected: "{'integer': 900}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_client_alive_count_max_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_client_alive_count_max_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_count_max_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_count_max_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_client_alive_count_max_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_count_max_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_client_alive_interval_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_client_alive_interval_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "900" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_interval_configure passed (Result: $result_value, Expected: "{'integer': 900}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_interval_configure failed (Result: $result_value, Expected: "{'integer': 900}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_client_alive_interval_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_client_alive_interval_configure failed (Result: $result_value, Expected: "{'integer': 900}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
echo $total
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_fips_compliant'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_fips_compliant'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "7" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_fips_compliant passed (Result: $result_value, Expected: "{'integer': 7}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_fips_compliant failed (Result: $result_value, Expected: "{'integer': 7}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_fips_compliant -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_fips_compliant failed (Result: $result_value, Expected: "{'integer': 7}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_login_grace_time_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_login_grace_time_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "30" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_login_grace_time_configure passed (Result: $result_value, Expected: "{'integer': 30}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_login_grace_time_configure failed (Result: $result_value, Expected: "{'integer': 30}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_login_grace_time_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_login_grace_time_configure failed (Result: $result_value, Expected: "{'integer': 30}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_permit_root_login_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_permit_root_login_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "no" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_permit_root_login_configure passed (Result: $result_value, Expected: "{'string': 'no'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_permit_root_login_configure failed (Result: $result_value, Expected: "{'string': 'no'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_permit_root_login_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_permit_root_login_configure failed (Result: $result_value, Expected: "{'string': 'no'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_unused_connection_timeout_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sshd_unused_connection_timeout_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "900" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_unused_connection_timeout_configure passed (Result: $result_value, Expected: "{'integer': 900}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_unused_connection_timeout_configure failed (Result: $result_value, Expected: "{'integer': 900}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sshd_unused_connection_timeout_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sshd_unused_connection_timeout_configure failed (Result: $result_value, Expected: "{'integer': 900}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sudoers_timestamp_type_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_sudoers_timestamp_type_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "tty" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_sudoers_timestamp_type_configure passed (Result: $result_value, Expected: "{'string': 'tty'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sudoers_timestamp_type_configure failed (Result: $result_value, Expected: "{'string': 'tty'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_sudoers_timestamp_type_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_sudoers_timestamp_type_configure failed (Result: $result_value, Expected: "{'string': 'tty'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_system_read_only'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_system_read_only'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "No" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_system_read_only passed (Result: $result_value, Expected: "{'string': 'no'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_system_read_only failed (Result: $result_value, Expected: "{'string': 'no'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_system_read_only -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_system_read_only failed (Result: $result_value, Expected: "{'string': 'no'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_tftpd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_tftpd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_tftpd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_tftpd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_tftpd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_tftpd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_time_server_enabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_time_server_enabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_time_server_enabled passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_time_server_enabled failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_time_server_enabled -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_time_server_enabled failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_touchid_prompt_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_touchid_prompt_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_touchid_prompt_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_touchid_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_touchid_prompt_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_touchid_prompt_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_unlock_active_user_session_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_unlock_active_user_session_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_unlock_active_user_session_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_unlock_active_user_session_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_unlock_active_user_session_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_unlock_active_user_session_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_user_app_installation_prohibit'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_user_app_installation_prohibit'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_user_app_installation_prohibit passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_user_app_installation_prohibit failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_user_app_installation_prohibit -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_user_app_installation_prohibit failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_uucp_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_uucp_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_uucp_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_uucp_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_uucp_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_uucp_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_account_lockout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_account_lockout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_account_lockout_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_account_lockout_timeout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_account_lockout_timeout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_timeout_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_account_lockout_timeout_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_account_lockout_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_alpha_numeric_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_alpha_numeric_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_alpha_numeric_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_alpha_numeric_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_alpha_numeric_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_alpha_numeric_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_custom_regex_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_custom_regex_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_custom_regex_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_custom_regex_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_custom_regex_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_custom_regex_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_history_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_history_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_history_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_history_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_history_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_history_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_max_lifetime_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_max_lifetime_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "60" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_max_lifetime_enforce passed (Result: $result_value, Expected: "{'integer': 60}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_max_lifetime_enforce failed (Result: $result_value, Expected: "{'integer': 60}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_max_lifetime_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_max_lifetime_enforce failed (Result: $result_value, Expected: "{'integer': 60}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_minimum_length_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_minimum_length_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_length_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_length_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_minimum_length_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_length_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_minimum_lifetime_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_minimum_lifetime_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_lifetime_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_lifetime_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_minimum_lifetime_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_minimum_lifetime_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_simple_sequence_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_simple_sequence_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_simple_sequence_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_simple_sequence_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_simple_sequence_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_simple_sequence_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_special_character_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('pwpolicy_special_character_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_special_character_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_special_character_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" pwpolicy_special_character_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - pwpolicy_special_character_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_airplay_receiver_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_airplay_receiver_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_airplay_receiver_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_airplay_receiver_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_airplay_receiver_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_airplay_receiver_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_apple_watch_unlock_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_apple_watch_unlock_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_apple_watch_unlock_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_apple_watch_unlock_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_apple_watch_unlock_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_apple_watch_unlock_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_automatic_login_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_automatic_login_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_login_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_login_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_automatic_login_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_login_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_automatic_logout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_automatic_logout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "86400" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_logout_enforce passed (Result: $result_value, Expected: "{'integer': 86400}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_logout_enforce failed (Result: $result_value, Expected: "{'integer': 86400}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_automatic_logout_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_automatic_logout_enforce failed (Result: $result_value, Expected: "{'integer': 86400}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_bluetooth_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_bluetooth_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_bluetooth_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_bluetooth_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_bluetooth_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_sharing_disable passed (Result: $result_value, Expected: "{'boolean': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_bluetooth_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_bluetooth_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_cd_dvd_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_cd_dvd_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_cd_dvd_sharing_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_cd_dvd_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_cd_dvd_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_cd_dvd_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_content_caching_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_content_caching_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_content_caching_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_content_caching_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_content_caching_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_content_caching_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_critical_update_install_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_critical_update_install_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_critical_update_install_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_critical_update_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_critical_update_install_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_critical_update_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_diagnostics_reports_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_diagnostics_reports_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_diagnostics_reports_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_diagnostics_reports_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_diagnostics_reports_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_diagnostics_reports_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$dontAllowDisable" == "true" ]] && [[ "$fileVault" == 1 ]]; then
  ...
  echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_filevault_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_filevault_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_filevault_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_filevault_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_filevault_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_filevault_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_find_my_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_find_my_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_find_my_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_find_my_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_find_my_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_find_my_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$profile" == "true" ]] && [[ "$plist" =~ [1,2] ]]; then
  ...
  echo "false"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_firewall_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_firewall_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_firewall_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$profile" == "true" ]] && [[ $plist == 1 ]]; then
  ...
  echo "false"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_firewall_stealth_mode_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_firewall_stealth_mode_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_stealth_mode_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_stealth_mode_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_firewall_stealth_mode_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_firewall_stealth_mode_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_gatekeeper_identified_developers_allowed'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_gatekeeper_identified_developers_allowed'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_identified_developers_allowed passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_identified_developers_allowed failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_gatekeeper_identified_developers_allowed -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_identified_developers_allowed failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_gatekeeper_override_disallow'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_gatekeeper_override_disallow'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_override_disallow passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_override_disallow failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_gatekeeper_override_disallow -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_gatekeeper_override_disallow failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_guest_access_smb_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_guest_access_smb_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_access_smb_disable passed (Result: $result_value, Expected: "{'boolean': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'boolean': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_guest_access_smb_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'boolean': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_guest_account_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_guest_account_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_account_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_account_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_guest_account_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_guest_account_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_hot_corners_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_hot_corners_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "4" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_disable passed (Result: $result_value, Expected: "{'integer': 4}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_disable failed (Result: $result_value, Expected: "{'integer': 4}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_hot_corners_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_disable failed (Result: $result_value, Expected: "{'integer': 4}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$bl_corner" != "6" ]] && [[ "$tl_corner" != "6" ]] && [[ "$tr_corner" != "6" ]] && [[ "$br_corner" != "6" ]]; then
  echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_hot_corners_secure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_hot_corners_secure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_secure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_hot_corners_secure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_improve_siri_dictation_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_improve_siri_dictation_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_improve_siri_dictation_disable passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_improve_siri_dictation_disable failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_improve_siri_dictation_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_improve_siri_dictation_disable failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_internet_accounts_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_internet_accounts_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_accounts_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_accounts_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_internet_accounts_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_accounts_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_internet_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_internet_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_sharing_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_sharing_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_internet_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_internet_sharing_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_location_services_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_location_services_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_location_services_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_location_services_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_location_services_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_location_services_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_loginwindow_prompt_username_password_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_loginwindow_prompt_username_password_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_loginwindow_prompt_username_password_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_loginwindow_prompt_username_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_loginwindow_prompt_username_password_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_loginwindow_prompt_username_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_media_sharing_disabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_media_sharing_disabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_media_sharing_disabled passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_media_sharing_disabled failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_media_sharing_disabled -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_media_sharing_disabled failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_password_hints_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_password_hints_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_password_hints_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_password_hints_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_password_hints_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_password_hints_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_personalized_advertising_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_personalized_advertising_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_personalized_advertising_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_personalized_advertising_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_personalized_advertising_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_personalized_advertising_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_printer_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_printer_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_printer_sharing_disable passed (Result: $result_value, Expected: "{'boolean': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_printer_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_printer_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_printer_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_rae_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_rae_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_rae_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_rae_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_rae_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_rae_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_remote_management_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_remote_management_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_remote_management_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_remote_management_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_remote_management_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_remote_management_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screen_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screen_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screen_sharing_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screen_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_screen_sharing_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screen_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_ask_for_password_delay_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_ask_for_password_delay_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_ask_for_password_delay_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_ask_for_password_delay_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_screensaver_ask_for_password_delay_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_ask_for_password_delay_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_password_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_password_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_password_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_screensaver_password_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_timeout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_screensaver_timeout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_timeout_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_screensaver_timeout_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_screensaver_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_siri_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_siri_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_siri_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_siri_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_siri_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_siri_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_smbd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_smbd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_smbd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_smbd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_smbd_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_smbd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_ssh_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_ssh_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_ssh_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_ssh_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_ssh_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_ssh_enable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_ssh_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
echo $result
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_system_wide_preferences_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_system_wide_preferences_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_system_wide_preferences_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_system_wide_preferences_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_system_wide_preferences_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_system_wide_preferences_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_time_server_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_time_server_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "time-a.nist.gov,time-b.nist.gov" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_configure passed (Result: $result_value, Expected: "{'string': 'time-a.nist.gov,time-b.nist.gov'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_configure failed (Result: $result_value, Expected: "{'string': 'time-a.nist.gov,time-b.nist.gov'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_time_server_configure -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_configure failed (Result: $result_value, Expected: "{'string': 'time-a.nist.gov,time-b.nist.gov'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_time_server_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_time_server_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_time_server_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_time_server_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_token_removal_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_token_removal_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_token_removal_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_token_removal_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_token_removal_enforce -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_token_removal_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_touchid_unlock_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_touchid_unlock_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_touchid_unlock_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_touchid_unlock_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_touchid_unlock_disable -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_touchid_unlock_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_usb_restricted_mode'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('system_settings_usb_restricted_mode'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_usb_restricted_mode passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_usb_restricted_mode failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" system_settings_usb_restricted_mode -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - system_settings_usb_restricted_mode failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_secure_enclave'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cnssi-1253_low.audit').objectForKey('os_secure_enclave'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cnssi-1253_low - os_secure_enclave passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cnssi-1253_low - os_secure_enclave failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/defaults write "$audit_plist" os_secure_enclave -dict-add finding -bool YES
            /usr/bin/logger "mSCP: cnssi-1253_low - os_secure_enclave failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
